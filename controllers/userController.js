...
const authenticateUser = async (req, res) => {
    ...
    if (!name || !password) return res.status(404).json({ message: "Please fill all the fields" })
    try {
        const user = await User.findOne({ name })
        ...
        return res.cookie('instaUser', token,{domain:'https://insta-frontend-six.vercel.app',secure:true}).status(200).json({ message: "User loged in successfully" })
    } catch (error) {
        return res.status(404).json({ message: "Something went wrong" })
    }
};
const deleteUser = async (req, res) => {
    ...
    try {
        ...
        return res.cookie('instaUser', '',{domain:'https://insta-frontend-six.vercel.app',secure:true}).status(200).json({ message: "User deleted  successfully" })
    } catch (err) {
        console.log(err);
        return res.status(404).json({ message: "Something went wrong" })
    }
...
const updateUserPassword = async (req, res) => {
    ...
    if (!name || !oldPassword || !newPassword) return res.status(404).json({ message: "Please fill all the fields" })
    try {
        const user = await User.findOne({ name })
        if (!user) return res.status(404).json({ message: "User not found" })
        ...
        return res.cookie('instaUser', '',{domain:'https://insta-frontend-six.vercel.app',secure:true}).status(200).json({ message: "Password Updated Successfully" })
    } catch (error) {
        console.log(error);
        return res.status(404).json({ message: "Something went wrong" })
    }
...
const updateUserDetails = async (req, res) => {
    ...
    if (!name || !hobby || !description || !fileObject) return res.status(404).json({ message: "Please fill all the fields" })
    ...
    try {
        const user = await User.findOne({ name })
        if (!user) return res.status(404).json({ message: "User not found" })
        file.mv('./public/users/' + fileName, async function (err) {
            if (err) {
                console.log(err);
                return res.status(404).json({ message: "Something went wrong" })
            } else {
...
const logOutUser = async (req, res) => {
    return res.cookie('instaUser', '',{domain:'https://insta-frontend-six.vercel.app',secure:true}).status(200).json({ message: "User loged out successfully" })
}
